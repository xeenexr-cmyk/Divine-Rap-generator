...
def generate_rap(style, topic, mood, length):
    try:
        # FIXED MODEL NAME (Latest Stable)
        model = genai.GenerativeModel('gemini-1.5-flash-latest')
        
        # Stable + Creative Config (Best for Rap)
        generation_config = {
            "temperature": 1.2,
            "top_p": 0.9,
            "top_k": 40,
        ...
        error_msg = str(e)
        if "404" in error_msg:
            return f"Error: Model not found. {error_msg}\n\nTip: Check if GEMINI_API_KEY is correct and model is available."
...
"""
                    ...
                    rhyme_score = "अच्छा"
                    if len(lines) >= 2:
                        last_word1 = lines[0].split()[-1] if lines[0].split() else ""
                        last_word2 = lines[1].split()[-1] if len(lines) > 1 and lines[1].split() else ""
                        if last_word1 and last_word2 and (last_word1 == last_word2 or last_word1[-2:] == last_word2[-2:]):
                            rhyme_score = "बहुत अच्छा"
